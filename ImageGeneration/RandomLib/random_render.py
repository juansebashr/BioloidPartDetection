...
from ImageGeneration.RandomLib.random_exceptions import ImprobableError
...
class ShellRingCoordinateDist(Distribution):
    ...
    def __init__(self, phi_sigma, normal, mu=90, **kwargs):
        """
        ...
        self.phi = TruncNormDist(mu=mu, sigmu=self.phi_sigma / 90.0, l=0.0, r=180.0)
    ...
    def change_param(self, param_name, param_val):
        ...
        if param_name == 'mu':
            ...
            self.phi.change_param('mu', param_val)
            self_dict[param_name] = param_val
            return
        ...
        if param_name == 'phi_sigma':
            ...
            self.phi.change_param('sigmu', param_val / self.mu)
            self_dict[param_name] = param_val
            return
        raise KeyError('Cannot find specified attribute!')
...
class CompositeShellRingDist(Distribution):
    ...
    def __init__(self, phi_sigma, normals, mu=90, **kwargs):
        """
        ...
        for normal in self.normals:
            self.distributions.append(ShellRingCoordinateDist(phi_sigma=self.phi_sigma, mu=self.mu, normal=normal))
    ...
    def change_param(self, param_name, param_val):
        ...
        if param_name == 'mu':
            self.mu = param_val
            for distribution in self.distributions:
                distribution.change_param('mu', param_val)
            return
